...
type ftRequestSignTransactionsOptions = {
  transactions: Transaction[];
  callbackUrl: string;
  meta?: string;
};

export class FTWalletConnection extends WalletConnection {
  public signTransactionUrl: string | null = null;

  async ftRequestSignIn(
    contractId: string,
    successUrl: string,
    failureUrl: string
    ...
    return new Promise<string | undefined>((resolve, reject) => {
      ...
      window.ftRequestSignInCompleted = async ({
        accountId,
        errorCode,
        allKeys,
        publicKey,
  ...
  async ftRequestSignTransactions({
    transactions,
    callbackUrl,
    meta,
  }: ftRequestSignTransactionsOptions): Promise<void> {
  ...
  account(): ConnectedWalletAccount {
    if (!this._connectedAccount) {
      this._connectedAccount = new FNConnectedWalletAccount(
        this,
